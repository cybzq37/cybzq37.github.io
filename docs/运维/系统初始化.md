...
echo "/dev/vdb1 /minedata xfs defaults 0 0 " >> /etc/fstab
...
```bash
# step1 挂载
mount /root/CentOS-7-x86_64-DVD-2009.iso /mirror
...
```bash
# 使用pgiz 多核压缩, 需要先安装
setsid bash -c 'tar cvf - raster | pigz -c | split -b 10G - raster.tar.gz.' > tar_split.log 2>&1 &

# 解压
cat raster.tar.gz.* | pigz -d | tar xvf -
cat raster.tar.gz.* | tar zxvf -
```

速度最快的压缩算法, 需要合并
```bash

tar -cvf - md-web-s14 | zstd --fast=3 -T0 -o md-web-s14.tar.zst

setsid bash -c 'tar -cvf - /minedata/lbs183 | zstd --fast=3 -T0  -o lbs183.tar.zst' > lbs183.log 2>&1 &

# 后台压缩
setsid bash -c 'tar -cvf - /minedata/lbs182 | zstd --fast=3 -T0 | split -b 2G - raster.tar.zst.' > raster.log 2>&1 &

# 解压缩
zstd -d -c lbs182.tar.zst | tar -xvf -

# 合并解压缩
cat images.tar.zst.* | zstd -d | tar -xf -
